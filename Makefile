...
local-run:
	DYNAMO_ENDPOINT=$(LOCAL_DYNAMO_ENDPOINT) AWS_REGION=$(AWS_REGION) DYNAMO_REGION=$(AWS_REGION) TICK_HZ=$(TICK_HZ) SPECTATOR_HZ=$(SPECTATOR_HZ) ENABLE_BROADCAST=$(ENABLE_BROADCAST) LOG_HZ=$(LOG_HZ) AWS_ACCESS_KEY_ID=local AWS_SECRET_ACCESS_KEY=local \
	DYNAMO_TABLE_USERS=$(LOCAL_DYNAMO_USERS) DYNAMO_TABLE_SNAKE_CHECKPOINTS=$(LOCAL_DYNAMO_SNAKE) DYNAMO_TABLE_EVENT_LEDGER=$(LOCAL_DYNAMO_EVENTS) DYNAMO_TABLE_SETTINGS=$(LOCAL_DYNAMO_SETTINGS) \
	./snake_server serve
...
aws-code-deploy:
	...
	DEPLOY_TIMEOUT_SEC=$(DEPLOY_TIMEOUT_SEC) TICK_HZ=$(TICK_HZ) SPECTATOR_HZ=$(SPECTATOR_HZ) ENABLE_BROADCAST=$(ENABLE_BROADCAST) LOG_HZ=$(LOG_HZ) AWS_PROFILE=$(PROFILE) AWS_REGION=$(AWS_REGION) PROJECT_TAG=$(PROJECT_TAG) ENVIRONMENT_TAG=$(ENVIRONMENT_TAG) ASG_NAME=$(ASG_NAME) APP_REF=$(BRANCH) BUILD_TARGET=$(BUILD_TARGET) DOMAIN_NAME=$(DOMAIN_NAME) APP_PORT=$(APP_PORT) bash infra/scripts/deploy_app.sh
